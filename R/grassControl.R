...
paramGRASSx <- function(set_default_GRASS = NULL, MP = "/usr/bin", ver_select = FALSE,
                        quiet = TRUE) {
  if (ver_select == "T")
    ver_select <- TRUE
  ...
  if (params_GRASS[[1]][1] != FALSE) {
    ...
    if (nrow(params_GRASS) == 1) {
    ...
    } else if (nrow(params_GRASS) > 1 & is.numeric(ver_select) & (ver_select > 0 & ver_select <=
                                                                  nrow(params_GRASS))) {
      if (!quiet) {
      ...
      if (!quiet) {
        cat("You have more than one valid GRASS version installed!\n")
        cat("The latest installed version (", which(params_GRASS$version == max(params_GRASS$version))[1],
            ")has been selected \n")
        print(params_GRASS)
        cat("\n")
      }
    ...
    # if a set_default_GRASS was provided take this } else { gisbase_GRASS <-
    # set_default_GRASS }
    grass <- list()
  ...
  if (params_GRASS[[1]][1] != FALSE) {
    ...
    if (nrow(params_GRASS) == 1) {
      ...
      gisbase_GRASS <- normalizePath(setenvGRASSw(root_GRASS = params_GRASS$instDir[[ver_select]],
                                                  grass_version = params_GRASS$version[[ver_select]], installation_type = params_GRASS$installation_type[[ver_select]],
                                                  quiet = quiet), winslash = "/")
      ...
      gisbase_GRASS <- setenvGRASSw(root_GRASS = params_GRASS$instDir[[which(params_GRASS$version ==
                                                                               max(params_GRASS$version))[1]]], grass_version = params_GRASS$version[[which(params_GRASS$version ==
                                                                                                                                                              max(params_GRASS$version))[1]]], installation_type = params_GRASS$installation_type[[which(params_GRASS$version ==
      ...
      installation_type = params_GRASS$installation_type[[which(params_GRASS$version ==
                                                                  max(params_GRASS$version))[1]]]
      ...
      gisbase_GRASS <- normalizePath(setenvGRASSw(root_GRASS = params_GRASS$instDir[[ver]],
                                                  grass_version = params_GRASS$version[[ver]], installation_type = params_GRASS$installation_type[[ver]],
                                                  quiet = quiet), winslash = "/")
    ...
    # if a set_default_GRASS was provided take this } else { gisbase_GRASS <-
    # setenvGRASSw(root_GRASS = set_default_GRASS[1], grass_version =
    # set_default_GRASS[2], installation_type = set_default_GRASS[3], quiet
    # =quiet) grass_version = set_default_GRASS[2] installation_type =
    # set_default_GRASS[3] params_GRASS<- data.frame(instDir = gisbase_GRASS,
    # version = grass_version, installation_type = installation_type,
...
searchGRASSW <- function(DL = "C:/", quiet = TRUE) {
  ...
  # trys to find a osgeo4w installation on the whole C: disk returns root
  # directory and version name recursive dir for grass*.bat returns all version of
  # grass bat files
  if (!quiet)
  ...
  # raw_GRASS <- try(system(paste0('cmd.exe /c dir /B /S ', DL,
  # '\\','grass*.bat'), intern = TRUE,ignore.stderr = TRUE))
  
  
  if (unique((grepl(raw_GRASS, pattern = "File not found") | grepl(raw_GRASS, pattern = "Datei nicht gefunden") |
              grepl(raw_GRASS, pattern = "INFORMATION:") | grepl(raw_GRASS, pattern = "FEHLER:") |
  ...
  if (!methods::is(raw_GRASS[1], "try-error")) {
    ...
    installations_GRASS <- lapply(seq(length(raw_GRASS)), function(i) {
      ...
      if (length(unique(grep(paste("OSGEO4W", collapse = "|"), batchfile_lines,
                             value = TRUE))) > 0) {
        osgeo4w <- TRUE
        stand_alone <- FALSE
      }
      # if the the tag 'NSIS installer' exists set installation_type
      if (length(unique(grep(paste("NSIS installer", collapse = "|"), batchfile_lines,
                             value = TRUE))) > 0) {
        osgeo4w <- FALSE
        stand_alone <- TRUE
      }
      ...
      if (osgeo4w) {
        if (basename(utils::shortPathName(raw_GRASS[i])) == "grass78.bat" || basename(utils::shortPathName(raw_GRASS[i])) ==
          ...
          # grep line with root directory and extract the substring defining
          # GISBASE
          root_dir <- dirname(dirname(utils::shortPathName(raw_GRASS[i])))
          # grep line with the version name and extract it
          ver_char <- substr(basename(utils::shortPathName(raw_GRASS[i])), 6,
                             7)
          installerType <- "osgeo4W"
        } else {
          if (length(grep("PREREM~1", utils::shortPathName(raw_GRASS[i]))) ==
              0 && length(grep("extrabin", utils::shortPathName(raw_GRASS[i]))) ==
              0) {
            # grep line with root directory and extract the substring
            # defining GISBASE
            root_dir <- unique(grep(paste("OSGEO4W_ROOT", collapse = "|"), batchfile_lines,
                                    value = TRUE))
            # if (substr(root_dir,1,1) == '\\' & length(root_dir) > 0)
            # root_dir <- substr(root_dir,3,nchar(root_dir))
            if (length(root_dir) > 0)
              root_dir <- substr(root_dir, gregexpr(pattern = "=", root_dir)[[1]][1] +
                                   1, nchar(root_dir))
            
            # grep line with the version name and extract it
            ver_char <- unique(grep(paste("\\benv.bat\\b", collapse = "|"), batchfile_lines,
                                    value = TRUE))
            if (length(root_dir) > 0) {
              ver_char <- substr(ver_char, gregexpr(pattern = "\\grass-", ver_char)[[1]][1],
                                 nchar(ver_char))
              ver_char <- substr(ver_char, 1, gregexpr(pattern = "\\\\", ver_char)[[1]][1] -
                                   1)
            }
          }
          installerType <- "osgeo4W"
      ...
      if (stand_alone) {
        ...
        root_dir <- unique(grep(paste("set GISBASE=", collapse = "|"), batchfile_lines,
                                value = TRUE))
      ...
      if (length(root_dir) > 0) {
        root_dir = root_dir <- substr(root_dir[[1]], gregexpr(pattern = "=", root_dir)[[1]][1] +
                                        1, nchar(root_dir))
      ...
      # put the existing GISBASE directory, version number and installation
      # type in a data frame
      if (length(root_dir) > 0 & exist) {
        data.frame(instDir = root_dir, version = ver_char, installation_type = installerType,
                   stringsAsFactors = FALSE)
      }
    })  # end lapply
...
searchGRASSX <- function(MP = "/usr/bin", quiet = TRUE) {
  if (MP == "default")
    MP <- "/usr/bin"
  raw_GRASS <- system2("find", paste(MP, " ! -readable -prune -o -type f -executable -iname 'grass??' -print"),
  ...
  if (length(raw_GRASS) > 0) {
    ...
    installations_GRASS <- lapply(seq(length(raw_GRASS)), function(i) {
      ...
      if (rg[[i]][lengths(rg)] == "grass78" | rg[[i]][lengths(rg)] == "grass") {
        ...
        ver_char <- grep(readLines(raw_GRASS[[i]]), pattern = "GRASS_VERSION = \"",
                         value = TRUE)
        ver_char <- substr(ver_char, gregexpr(pattern = "\"", ver_char)[[1]][1] +
                             1, nchar(ver_char) - 1)
        cmd <- grep(readLines(raw_GRASS[[i]]), pattern = "CMD_NAME = \"", value = TRUE)
        cmd <- substr(cmd, gregexpr(pattern = "\"", cmd)[[1]][1] + 1, nchar(cmd) -
                        1)
        
        rootdir <- grep(readLines(raw_GRASS[[i]]), pattern = "GISBASE = os.path.normpath",
                        value = TRUE)
        root_dir <- substr(rootdir[2], gregexpr(pattern = "\"", rootdir[2])[[1]][1] +
                             1, nchar(rootdir[2]) - 2)
        ...
        root_dir <- try(grep(readLines(raw_GRASS[[i]]), pattern = "gisbase = \"",
      ...
      data.frame(instDir = root_dir, version = ver_char, installation_type = cmd,
                 stringsAsFactors = FALSE)
...
setenvGRASSw <- function(root_GRASS = NULL, grass_version = NULL, installation_type = NULL,
  ...
  if (Sys.info()["sysname"] == "Windows") {
    ...
    if (installation_type == "osgeo4W" || installation_type == "OSGeo4W64") {
      ...
      Sys.setenv(PYTHONHOME = paste0(Sys.getenv("OSGEO4W_ROOT"), "\\apps\\Python27"),
                 envir = .GRASS_CACHE)
      Sys.setenv(PYTHONPATH = paste0(Sys.getenv("OSGEO4W_ROOT"), "\\apps\\grass\\",
                                     grass_version, "\\etc\\python"), envir = .GRASS_CACHE)
      ...
      Sys.setenv(FONTCONFIG_FILE = paste0(Sys.getenv("OSGEO4W_ROOT"), "\\bin\\gdalplugins"),
                 envir = .GRASS_CACHE)
      ...
      Sys.setenv(PATH = paste0(gisbase_GRASS, ";", root_GRASS, "\\apps\\Python27\\lib\\site-packages\\numpy\\core",
                               ";", root_GRASS, "\\apps\\grass\\", grass_version, "\\bin", ";", root_GRASS,
                               "\\apps\\grass\\", grass_version, "\\lib", ";", root_GRASS, "\\apps\\grass\\",
                               grass_version, "\\etc", ";", root_GRASS, "\\apps\\grass\\", grass_version,
                               "\\etc\\python", ";", root_GRASS, "\\apps\\Python27\\Scripts", ";", root_GRASS,
                               "\\bin", ";", root_GRASS, "\\apps", ";", paste0(Sys.getenv("WINDIR"),
      ...
      Sys.setenv(GRASS_PYTHON = paste0(Sys.getenv("GRASS_ROOT"), "\\bin\\python.exe"),
                 envir = .GRASS_CACHE)
      Sys.setenv(PYTHONHOME = paste0(Sys.getenv("GRASS_ROOT"), "\\Python27"), envir = .GRASS_CACHE)
      Sys.setenv(PYTHONPATH = paste0(Sys.getenv("GRASS_ROOT"), "\\etc\\python"),
                 envir = .GRASS_CACHE)
      Sys.setenv(GRASS_PROJSHARE = paste0(Sys.getenv("GRASS_ROOT"), "\\share\\proj"),
                 envir = .GRASS_CACHE)
      Sys.setenv(PROJ_LIB = paste0(Sys.getenv("GRASS_ROOT"), "\\share\\proj"), envir = .GRASS_CACHE)
      Sys.setenv(GDAL_DATA = paste0(Sys.getenv("GRASS_ROOT"), "\\share\\gdal"),
                 envir = .GRASS_CACHE)
      Sys.setenv(GEOTIFF_CSV = paste0(Sys.getenv("GRASS_ROOT"), "\\share\\epsg_csv"),
                 envir = .GRASS_CACHE)
      Sys.setenv(FONTCONFIG_FILE = paste0(Sys.getenv("GRASS_ROOT"), "\\etc\\fonts.conf"),
                 envir = .GRASS_CACHE)
      Sys.setenv(JPEGMEM = jpgmem, envir = .GRASS_CACHE)
      Sys.setenv(FONTCONFIG_FILE = paste0(Sys.getenv("GRASS_ROOT"), "\\bin\\gdalplugins"),
                 envir = .GRASS_CACHE)
      Sys.setenv(GISRC = paste(Sys.getenv("HOME"), "\\.grassrc7", sep = ""), envir = .GRASS_CACHE)
      ...
      Sys.setenv(PATH = paste0(gisbase_GRASS, ";", root_GRASS, "\\Python27\\lib\\site-packages\\numpy\\core",
                               ";", root_GRASS, "\\bin", ";", root_GRASS, "\\extrabin", ";", root_GRASS,
                               "\\lib", ";", root_GRASS, "\\etc", ";", root_GRASS, "\\etc\\python", ";",
                               root_GRASS, "\\Scripts", ";", root_GRASS, ";", paste0(Sys.getenv("WINDIR"),
                                                                                     "/WBem"), ";", Sys.getenv("PATH")), envir = .GRASS_CACHE)
...
checkGisdbase <- function(x = NULL, gisdbase = NULL, location = NULL, gisdbase_exist = FALSE,
                          obj_name = NULL) {
  if (gisdbase_exist)
...
findGRASS <- function(searchLocation = "default", ver_select = FALSE, quiet = TRUE) {
  ...
  if (Sys.info()["sysname"] == "Windows") {
    ...
    if (grepl(paste0(LETTERS, ":", collapse = "|"), substr(toupper(searchLocation),
                                                           start = 1, stop = 2))) {
