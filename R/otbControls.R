...
searchOTBW <- function(DL = "default", quiet = TRUE) {
  ...
  if (Sys.info()["sysname"] == "Windows") {
    ...
    # trys to find a osgeo4w installation on the whole C: disk
    # returns root directory and version name recursive dir for
    # otb*.bat returns all version of otb bat files
    if (!quiet)
    ...
    raw_OTB <- try(system(paste0("cmd.exe /c WHERE /R ", DL, " ", "otbcli.bat"),
                          intern = TRUE))
    
    # raw_OTB <- try(system(paste0('cmd.exe',' /c dir /B /S
    # ',DL,'\\','otbcli.bat'),intern=TRUE))
    if (identical(raw_OTB, character(0)))
      raw_OTB <- "File not found"
    if (grepl(raw_OTB[1], pattern = "File not found") | grepl(raw_OTB[1],
                                                              pattern = "Datei nicht gefunden") | grepl(raw_OTB[1], pattern = "INFORMATION:") |
        grepl(raw_OTB[1], pattern = "FEHLER:") | grepl(raw_OTB[1],
                                                       pattern = "ERROR:")) {
    ...
    if (class(raw_OTB)[1] != "try-error") {
      ...
      # trys to identify valid otb installations and their
      # version numbers
      otbInstallations <- lapply(seq(length(raw_OTB)), function(i) {
        ...
        installerType <- ""
        # if the the tag 'OSGEO4W64' exists set
        # installation_type
        if (length(unique(grep(paste("OSGeo4W64", collapse = "|"),
                               raw_OTB[i], value = TRUE))) > 0) {
          root_dir <- unique(grep(paste("OSGeo4W64", collapse = "|"),
                                  raw_OTB[i], value = TRUE))
          root_dir <- substr(root_dir, 1, gregexpr(pattern = "otbcli.bat",
                                                   root_dir)[[1]][1] - 1)
          installDir <- substr(root_dir, 1, gregexpr(pattern = "bin",
                                                     root_dir)[[1]][1] - 2)
          installerType <- "osgeo4w64OTB"
        } else if (length(unique(grep(paste("OSGeo4W", collapse = "|"),
                                      raw_OTB[i], value = TRUE))) > 0) {
          # if the the tag 'OSGEO4W' exists set
          # installation_type
          root_dir <- unique(grep(paste("OSGeo4W", collapse = "|"),
                                  raw_OTB[i], value = TRUE))
          root_dir <- substr(root_dir, 1, gregexpr(pattern = "otbcli.bat",
                                                   root_dir)[[1]][1] - 1)
          installDir <- substr(root_dir, 1, gregexpr(pattern = "bin",
        ...
        } else if (length(unique(grep(paste("QGIS", collapse = "|"),
                                      batchfile_lines, value = TRUE))) > 0) {
          # if the the tag 'QGIS' exists set
          # installation_type
          root_dir <- unique(grep(paste("QGIS", collapse = "|"),
                                  raw_OTB[i], value = TRUE))
          root_dir <- substr(root_dir, 1, gregexpr(pattern = "otbcli.bat",
                                                   root_dir)[[1]][1] - 1)
          installDir <- substr(root_dir, 1, gregexpr(pattern = "bin",
        ...
        } else if (length(unique(grep(paste("OTB", collapse = "|"),
                                      batchfile_lines, value = TRUE))) > 0) {
          # if the the tag 'OTB-' exists set
          # installation_type
          root_dir <- unique(grep(paste("OTB", collapse = "|"),
                                  raw_OTB[i], value = TRUE))
          root_dir <- substr(root_dir, 1, gregexpr(pattern = "otbcli.bat",
                                                   root_dir)[[1]][1] - 1)
          installDir <- substr(root_dir, 1, gregexpr(pattern = "bin",
        ...
        # put the existing binary root directory in a data
        # frame data.frame(binDir = root_dir, baseDir =
        # installDir, installation_type = installerType,
        # stringsAsFactors = FALSE)
        data.frame(binDir = root_dir, baseDir = installDir, otbCmd = paste0(root_dir,
                                                                            "otbcli"), stringsAsFactors = FALSE)
      })  # end lapply
      # bind the df lines
      ...
      if (!quiet)
        cat("Did not find any valid OTB installation at mount point",
            DL)
...
searchOTBX <- function(MP = "default", quiet = TRUE) {
  ...
  if (!exists("GiEnv"))
    GiEnv <- new.env(parent = globalenv())
  # trys to find a osgeo4w installation at the mounting point disk
  # returns root directory and version name recursive dir for
  # otb*.bat returns all version of otb bat files
  if (!quiet)
    cat("\nsearching for Orfeo Toolbox installations - this may take a while\n")
  ...
  options(warn = -1)
  raw_OTB <- try(system2("find", paste(MP, " ! -readable -prune -o -type f -executable -iname 'otbcli' -print"),
                         stdout = TRUE))
  if (identical(raw_OTB, character(0))) {
    # raw_OTB <- 'File not found' if (grepl(raw_OTB,pattern =
    # 'File not found') | grepl(raw_OTB,pattern = 'Datei nicht
    # gefunden')) {
  ...
  if (class(raw_OTB)[1] != "try-error") {
    ...
    # installation found') trys to identify valid otb
    # installations and their version numbers
    otbInstallations <- lapply(seq(length(raw_OTB)), function(i) {
      
      # TODO strip version from OTB /usr/bin/otbcli_BandMath
      # -version 'This is the BandMath application, version
      # 6.0.0'
      
      # if the the tag 'OSGEO4W64' exists set installation_type
      root_dir <- substr(raw_OTB[i], 1, gregexpr(pattern = "otbcli",
                                                 raw_OTB[i])[[1]][1] - 1)
      # put the existing GISBASE directory, version number and
      # installation type in a data frame
      data.frame(binDir = root_dir, otbCmd = paste0(root_dir, "otbcli"),
                 stringsAsFactors = FALSE)
    })  # end lapply
    # bind the df lines
    otbInstallations <- do.call("rbind", otbInstallations)
...
getrowotbVer <- function(paths) {
  ...
  scmd = ifelse(Sys.info()["sysname"] == "Windows", "otbcli_LocalStatisticExtraction.bat",
  ...
  for (i in 1:length(paths)) {
    if (file.exists(paste0(paths[i], "../VERSION")))
      tmp = strsplit(grep("OTB Version", readLines(paste0(paths[i],
                                                          "../VERSION")), value = TRUE), "OTB Version: ")[[1]][2] else if (grepl("OTB-", paths[i]))
                                                            tmp = substr(strsplit(paths[i], "OTB-")[[1]][2], start = 1,
                                                                         stop = 5)
...
getotbVer <- function(paths) {
  ...
  otbVersion <- strsplit(x = system(paste0(paste0(shQuote(paths), sep,
                                                  scmd), " -version"), intern = FALSE), split = " version ")[[1]][2]
