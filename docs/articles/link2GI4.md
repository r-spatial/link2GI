...
applications for remote sensing and image processing. `link2GI` wraps
these applications in R with a focus on **reproducibility, transparency,
and robustness across OTB versions**.
...
then executes it. Because the legacy parser may expose parameter names
that vary between OTB versions/apps, the safe pattern is:
...
- [`otb_build_cmd()`](https://r-spatial.github.io/link2GI/reference/otb_api.md)
  prevents invalid keys (reduces “parameter does not exist” failures)
- [`runOTB()`](https://r-spatial.github.io/link2GI/reference/runOTB.md)
  on Linux/macOS uses the OTB launcher plus explicit environment,
  matching the help/spec stack
- Windows isolation is handled by
...
- [`parseOTBFunction()`](https://r-spatial.github.io/link2GI/reference/parseOTBFunction.md)
  is legacy (supported, but not recommended for new code).
- [`parseOTBAlgorithms()`](https://r-spatial.github.io/link2GI/reference/parseOTBAlgorithms.md)
  is legacy (if present in your build; prefer
  [`otb_capabilities()`](https://r-spatial.github.io/link2GI/reference/otb_api.md)).
